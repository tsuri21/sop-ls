...
{
    mode_t masks[] = { S_IRUSR, S_IWUSR, S_IXUSR, S_IRGRP, S_IWGRP, S_IXGRP, S_IROTH, S_IWOTH, S_IXOTH};
    char chars[] = "rwx";
    char *labels[] = {"USER:", " GROUP:", " OTHERS:"};

    for (int i = 0; i < 9; i++){
        if (opts.human_readable && i%3==0){
            printf(ANSI_COLOR_RESET "%s", labels[i/3]);
        }
        if (info->st_mode && masks[i]) {
            if (opts.human_readable) {
                const char *colors[] = {ANSI_COLOR_GREEN, ANSI_COLOR_YELLOW , ANSI_COLOR_RED};
                printf( "%s%c" ANSI_COLOR_RESET, colors[i % 3], chars[i % 3]);
            }
            else {
                printf(ANSI_COLOR_RESET "%c", chars[i % 3]);
            }
        }
        else {
            putchar('-');
